...
-- Q14) '전주' 지역 고객의 아이디를 조회하세요.
    select customer_id, city from customer where city = '전주';
-- Q15) 위 두 쿼리문을 조합해서 하위 쿼리를 사용해 '전주' 지역 고객 주문의 주문번호, 고객아이디, 주문일시, 전체금액을 조회하세요.
	select order_id, o.customer_id, order_date, total_due, city from order_header o inner join customer c on c.customer_id = o.customer_id and city = '전주';
    select order_id, o.customer_id, order_date, total_due from order_header o where customer_id in (select customer_id from customer where city = '전주';
-- Q16) 고객의 포인트 최댓값을 조회하세요.
	select customer_name, max(point) from customer group by customer_name;
-- Q17) 하위 쿼리를 사용해 가장 포인트가 많은 고객의 이름, 아이디, 등록일, 포인트를 조회하세요.
	select customer_name,customer_id,point from customer where point = (select max(point) from customer );
-- Q18) 하위 쿼리를 사용해 홍길동(gdhong) 고객보다 포인트가 많은 고객 이름, 아이디, 등록일, 포인트를 조회하세요.
	select customer_name,customer_id,point from customer where point > (select point from customer where customer_name = '홍길동');
-- Q19) 하위 쿼리를 사용해 홍길동(gdhong) 고객과 같은 지역의 고객 이름, 아이디, 지역, 등록일, 포인트를 조회하세요.
--      단, 고객 이름을 기준으로 오름차순 정렬해서 조회하세요.
	select customer_name,customer_id,city, point from customer where point > (select point from customer where customer_name = '홍길동') and city = (select city from customer where customer_name = '홍길동') order by customer_name asc;
-- Q20) 하위 쿼리를 사용해 홍길동(gdhong) 고객보다 포인트가 많은 고객 이름, 아이디, 등록일, 포인트를 조회하고, 행번호를 추가하여 출력하세요.
	select row_number() over(order by customer_id) as number,customer_name,customer_id,city from (select customer_name,customer_id,city, point from customer where point > (select point from customer where customer_name = '홍길동') and city = (select city from customer where customer_name = '홍길동') order by customer_name asc) a;
