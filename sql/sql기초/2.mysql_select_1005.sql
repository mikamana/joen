...
select eng_name, hire_date, phone, salary from employee where gender = 'M';
-- NULL은 미지수의 개념으로 이해!! 
...
select ifnull(eng_name,'홍길동') as retire_name from employee;
...
-- 순서로 정렬하여 조회
select * from employee where dept_id = 'sys' order by hire_date, salary;
select * from employee inner join department on department.dept_id = employee.dept_id where employee.dept_id = 'sys';
