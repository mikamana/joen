...
-- 홍길동 사원의 사원아이디, 성별, 폰번호, 이메일, 연봉 조회
	select emp_id, emp_name, gender, phone, email, salary from employee where emp_name = '홍길동';
...
-- 입사일이 2016년 1월 1일 이후 이고, 연봉이 6000 이상인 사원들을 조회
	select * from employee where hire_date >= '2016-01-01' && salary >= '6000';
-- 최근 입사일 기준으로 정렬
	select * from employee order by hire_date DESC;
-- 입사일이 2016년 1월 1일 이후 이거나, 연봉이 6000 이상인 사원들을 조회
	select * from employee where hire_date >= '2016-01-01' or salary >= '6000';
-- 정보시스템 부서 중 급여가 7000 이상인 사원들만 조회
	select * from employee where dept_id = 'sys' and salary >= '7000';
-- 영업부 직원들의 사원아이디, 사원명, 성별, 입사일, 이메일
	select emp_id, emp_name, gender, hire_date, email, dept_id from employee where dept_id = 'mkt';
-- 연봉, 보너스(연봉의 20%) 조회
-- 연봉이 null인 사원은 기본값을 50으로 정의
-- 보너스가 1000 이상인 사원 조회
	select salary, ifnull(salary*0.2,50) as bonus from employee where salary*0.2 >= 1000; 
...
-- 사원아이디, 사원명, 입사년도, 폰번호, 이메일 조회
	select emp_id, emp_name, hire_date, phone, email from employee;
 -- 2015년도 입사자들 모두 조회
	select * from employee where substring(hire_date,1,4) = '2015';
 -- 1월에 입사한 사원들 조회
...
-- 123456789 숫자를 문자로 변환 후 3자리숫자로 구분
	select format(cast(123456789 as char),0) as 숫자;
...
-- 단, 소수점이 있는 경우 모두 절삭
	select concat(format(floor(avg(salary)),0),'만원') from employee;
-- 총급여, 평균급여 출력
	select concat(format(sum(salary),0),'만원'),concat(format(floor(avg(salary)),0),'만원') from employee;
-- 총연봉, 평균연봉, 최대연봉, 최저연봉      
	select concat(format(sum(salary),0),'만원') as 총연봉 ,concat(format(floor(avg(salary)),0),'만원') as 평균연봉, concat(format(max(salary),0),'만원') as 최대연봉, concat(format(max(salary),0),'만원') as 최저연봉 from employee;

-- 가장빠른 입사일, 가장최근 입사일
	select min(hire_date) as 빠른입사, max(hire_date) as 느린입사 from employee;

-- 부서별 총연봉, 평균연봉, 최대연봉, 최저연봉
	select dept_id, concat(format(sum(salary),0),'만원') as 총연봉 ,concat(format(floor(avg(salary)),0),'만원') as 평균연봉, concat(format(max(salary),0),'만원') as 최대연봉, concat(format(max(salary),0),'만원') as 최저연봉 from employee group by dept_id;
-- 기본키의 group by는 개별적인 데이터기준으로 그룹핑됨
	
-- 입사년도별 총연봉, 평균연봉, 최대연봉, 최저연봉
	select substring(hire_date,1,4), concat(format(sum(salary),0),'만원') as 총연봉 ,concat(format(floor(avg(salary)),0),'만원') as 평균연봉, concat(format(max(salary),0),'만원') as 최대연봉, concat(format(max(salary),0),'만원') as 최저연봉 from employee group by substring(hire_date,1,4);
-- 입사월별 사원수, 총연봉, 평균연봉, 최대연봉, 최저연봉
	select substring(hire_date,6,2), count(*), concat(format(sum(salary),0),'만원') as 총연봉 ,concat(format(floor(avg(salary)),0),'만원') as 평균연봉, concat(format(max(salary),0),'만원') as 최대연봉, concat(format(max(salary),0),'만원') as 최저연봉 from employee group by substring(hire_date,6,2);
-- 부서별 사원수가 2명인 부서만 출력
	select dept_id from employee group by dept_id having count(*) = 2;
 -- 입사년도별 평균급여가 5000 이상인 해당년도만 출력
	select substring(hire_date,1,4), concat(floor(avg(salary)),'만원') from employee group by substring(hire_date,1,4) having avg(salary) >= 5000;
  -- 부서별 최고급여가 6000 이상 7000미만인 부서 출력
	select dept_id, max(salary) as 최고급여 from employee group by dept_id having 최고급여 between 6000 and 6999;
 -- rollup 함수를 사용하여 총합계, 소계를 출력
	select dept_id, sum(salary) from employee group by dept_id with rollup;
 -- 부서별 총합계를 출력, 3자리 구분
	select dept_id, format(sum(salary),0) from employee where salary is not null group by dept_id; 
 -- 단, 사원이 없는 부서는 그룹핑하지 않음
 -- 년도별, 각 부서의 사원수와 총연봉을 출력
	select substring(hire_date,1,4) as 년도 , dept_id, count(*), sum(salary) from employee group by 년도, dept_id;
...
-- 부서별 총연봉 출력하고, rollup을 이용하여 그룹핑한 부서의 총합계를 마지막에 '총합계'라는 컬럼명으로 출력
	select sum(salary) 총합계 ,if(grouping(dept_id),'총합계',ifnull(dept_id,'-')) 부서 from employee where salary is not null group by dept_id with rollup;
...
-- 2018년에 입사한 영업부, 2015년에 입사한 정보시스템 부서, 2014년에 입사한 사원들의 사원아이디, 사원명, 부서아이디, 입사일을 union을 사용하여 출력해주세요
    ...
    union
    select  substring(hire_date,1,4) as 입사일, dept_id,emp_name,emp_id from employee where substring(hire_date,1,4) = '2014';
	select * from employee;
    select * from department;
-- 2016년에 입사한 영업부, 2015년에 입사한 정보시스템 부서, 2014년에 입사한 사원들의 사원아이디, 사원명, 부서아이디, 입사일을 출력하고 
-- union all을 사용하여 출력해주세요
		select  substring(hire_date,1,4) as 입사일, dept_id,emp_name,emp_id from employee where substring(hire_date,1,4) = '2016' and dept_id = 'mkt'
    union all
    select  substring(hire_date,1,4) as 입사일, dept_id,emp_name,emp_id from employee where substring(hire_date,1,4) = '2015' and dept_id = 'sys'
    union all
    select  substring(hire_date,1,4) as 입사일, dept_id,emp_name,emp_id from employee where substring(hire_date,1,4) = '2014';
