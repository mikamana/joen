...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const initEvent = () => {
    ...
    document.querySelector('.email_select').addEventListener("change", e => {

      document.querySelector(".email_checked").value = e.target.value
      emailCheck();

    })// 이메일 체크 벨류 가져오는 이벤트

    document.querySelector(".address_btn").addEventListener("click", e => {
    ...
    document.querySelector(".input_wrap .id_btn").addEventListener("click", e => {

      let idValue = document.querySelector(".input_wrap .id").value
    ...
    document.querySelector(".password").addEventListener("input", e => {
      ...
      if (e.target.value.length >= 4 && e.target.value.length < 12) {
        ...
        document.querySelector('.err_password').textContent = "사용 가능한 비밀번호 입니다."
        ...
        document.querySelector('.err_password').textContent = "사용 불가능한 비밀번호 입니다."
    ...
    document.querySelector(".password_check").addEventListener("input", e => {
      ...
      if (pwValue === e.target.value) {
        document.querySelector('.err_password_check').textContent = "입력하신 비밀번호와 같습니다."
        document.querySelector('.err_password_check').style.color = "green"
      } else if (!e.target.value) {
        document.querySelector('.err_password_check').textContent = ""
      } else {
        document.querySelector('.err_password_check').textContent = "입력하신 비밀번호와 다릅니다."
        document.querySelector('.err_password_check').style.color = "red"
      }
...
const form_check = () => {
  ...
  let emailCheck = document.querySelector(".input_wrap .email_checked")
  ...
  let select = document.querySelector(".email_select")
  ...
  if (pwCheck.value !== password.value) {
    document.querySelector('.err_password_check').textContent = "입력한 비밀번호가 다릅니다."
    document.querySelector('.err_password_check').style.color = "red"
    pwCheck.focus()
    return false;
  } else {
  ...
  if (birthday.value.length <= 7 || birthday.value.length > 8) {
    document.querySelector('.err_birthday').textContent = "8자리 형식으로 입력해주세요."
    birthday.focus()
    return false;
  } else {
    removeFnErrorTextColor("err_birthday")
  ...
  if ((phone.value).includes('-')) {
    document.querySelector('.err_phone').textContent = " * '-' 없이 숫자만 입력"
  ...
  if (!checkbox1.checked) {
    document.querySelector(".err_checked1").textContent = "체크해주세요"
    document.querySelector(".err_checked1").style.color = "red"
    checkbox1.focus()
    return false;
  } else {
  ...
  document.querySelector('form.input_wrap').submit();
...
const emailCheck = () => {
  ...
  let emailSel = document.querySelector('.email_select');
  let emailDns = document.querySelector('.email_checked');
